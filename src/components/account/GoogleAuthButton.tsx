...
export function GoogleAuthButton({ label = 'Continuar con Google' }: { label?: string }) {
  ...
  async function handleClick() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/auth/callback?redirect_to=/cuenta` }
    })
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="flex w-full items-center justify-center gap-3 rounded-full border-2 border-gray-light bg-white px-7 py-3 font-display text-[15px] font-bold text-charcoal transition hover:border-charcoal disabled:opacity-60"
    >
      <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden>
        <path
          fill="#4285F4"
          d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84c-.21 1.13-.84 2.08-1.8 2.72v2.26h2.9c1.7-1.56 2.7-3.87 2.7-6.62Z"
        />
        <path
          fill="#34A853"
